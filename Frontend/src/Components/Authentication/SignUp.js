...
class SignUp extends Component {
    ...
    registerUser = (event) => {
        ...
        // if (/^[A-Za-z0-9]*$/.test(this.state.fname)) {

        // } else {
        //     message.error("NickName Invalid. Should contain only Alphabets and Numbers");
        //     return;
        // }
        var data = {
    ...
    validateDetails = (event) => {
        if (!this.state.invalidEmail && !this.state.invalidPassword && this.state.passwordMatch && this.state.fname !== "" ) return false
        else return true
    }
    handleDialogClose = () => {
        this.setState({
